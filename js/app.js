class UI {
  ...
  if(expenseValue === "" || amountValue ==="" || amountValue<0){
    ...
    this.expenseFeedback.innerHTML = `<p>value cannot be empty ot negative</p>`
    const self = this;
    setTimeout(function()
    {
...
function eventListners(){
  ...
  budgetForm.addEventListener("submit", function(event){
   event.preventDefault(); //to prevent automatic submitting
   ui.submitBudgetForm();
  });
  ...
  expenseList.addEventListener("click", function(event){
     if(event.target.parentElement.classList.contains("edit-icon")){
         ui.edit.Expense(event.target.parentElement)
     }
